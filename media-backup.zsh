...
do_sync() {
    ...
    if [[ -n "$lines" ]]; then
        ...
        while IFS= read -r line; do
            ...
            local trimline=$(echo "${line}" | cut -c 14-)
            [[ -n "${trimline}" ]] && echo "    /${trimline}"
        done <<< "$lines"
    else
