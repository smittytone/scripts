...
source_file="$GH/dotfiles/Mac/keyscripts"
scripts_dir="$GH/scripts"
repos=()
states=()
versions=()
...
Options:
  -l / --list           Just list scripts current states
  -q / --quiet          Don't show script version and state information
  -v / --version        Display the utility's version and exit
  -t / --target [path]  Specify an install directory. Default: /usr/local/bin
  -f / --file   [path]  Specify a script-list file. Default: [git]/dotfiles/Mac/keyscripts
  -s / --source [path]  Specify a script source directory. Default: [git]/scripts
...
if [[ ! -e "${bin_dir}" ]]; then
    if mkdir -p "${bin_dir}" print_err_and_exit "Could not create ${bin_dir} -- exiting"
fi
    ...
    if [ -e "${scripts_dir}" ]; then
        ...
        while IFS= read -r line; do
            ...
            if [ "${do_list}" -eq 0 ]; then
                ...
                # FROM 1.7.0 -- construct source path for new scripts/ structure
                case "${line:t:e}" in
                    py) sub_dir="python" ;;
                    sh) sub_dir="bash" ;;
                    zsh) sub_dir="zsh" ;;
                ...
                if [ -f "${scripts_dir}/${sub_dir}/${line}" ]; then
                    if [[ -f "${target_file}" ]] diff_result=$(diff ${target_file} "${scripts_dir}/${sub_dir}/${line}")

                    # FROM 1.0.1
                    # Only copy if the file is different
                    if [ -n "${diff_result}" ]; then
                        cp "${scripts_dir}/${sub_dir}/${line}" "${target_file}"
                ...
                if [[ -e "${target_file}" ]] get_version ${target_file} N
            fi
        done < "${source_file}"
    else
...
if [ "${do_show}" -eq 1 ]; then
    if [ "${#repos[@]}" -gt 0 ]; then
        if [[ "${list_err}" -eq 1 ]] printf "\n"
        print_mid ${name_max} ${ver_max} 11
        if [ "${do_list}" -eq 0 ]; then
