...
# Prep Apple Watch App Icons and Complication Icons
#
# Version 1.0.0
...
argIsAValue=0
args=(-p)
asizes=(216 196 172 100 88 87 80 58 55 48)
csizes=(224 203 182 64 58 52 50 44 40 36 32)
target="$HOME"/Desktop
...
if [ -f "$path" ]; then
    ...
    if [[ $ext_test = "PNG" || $ext_test = "JPG" || $ext_test = "JPEG" ]]; then
        ...
        for size in ${csizes[@]}; do
            newname="$target"/comp_"$size.$extension"
            cp "$path" "$newname"
            sips "$newname" -Z "$size" -i > /dev/null
        done
    else
